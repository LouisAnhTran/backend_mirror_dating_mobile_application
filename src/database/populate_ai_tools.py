...
async def insert_data():
    ...
    for _, row in df.iterrows():
        try:
            await conn.execute("""
                INSERT INTO ai_tools (tool_name, tool_url, icon_url, category, description, performance, popularity, security, ease_of_use, api_support)
            ...
            row['performance'], row['popularity'], row['security'], row['ease_of_use'], True if int(row['api_support'])==1 else False)
